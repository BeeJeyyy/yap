...
export default function Funnyrandom() {
    ...
    async function loadQuestions() {
      try {
        ...
        const res = await fetch("/api/generate-questions", {
          ...
          body: JSON.stringify({ topic: "funny" }),
        });
      ...
      <div className="px-4 sm:px-8 md:px-16 lg:px-52">
        ...
        <div className="relative flex items-center py-12">
          ...
          <div className="absolute left-1/2 -translate-x-1/2 flex items-center gap-2">
            <Badge className="bg-deck-family p-0 w-2 h-2" />
            <p className="font-bold tracking-wide">Funny & Random</p>
          </div>
        </div>
        ...
        <div>
          ...
          {!isLoading && !error && !isMaxReached && questions.length > 0 && (
            <SwipeCard deck="funny" question={questions[currentIndex]} />
          )}
        </div>
