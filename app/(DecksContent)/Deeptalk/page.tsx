  ...
  useEffect(() => {
    async function loadQuestions() {
      ...
      try {
        ...
        const data = await fetchQuestions("deeptalk", user.id);
        setQuestions(data);
      } catch(err) {
    ...
    if (!authLoading && user && !hasLaoded.current) {
      hasLaoded.current = true;
      loadQuestions();
    }
  }, [authLoading, user]);
      ...
      <div className="px-4 sm:px-8 md:px-16 lg:px-52">
        ...
        <div className="relative flex items-center py-12">
          ...
          <div className="absolute left-1/2 -translate-x-1/2 flex items-center gap-2">
            <Badge className="bg-deck-deep p-0 w-2 h-2" />
            <p className="font-bold tracking-wide">Deep Talk</p>
          </div>
        </div>
        ...
        <div>
          ...
          {!isLoading && !error && !isMaxReached && currentQuestion && (
            <SwipeCard deck="deeptalk" question={currentQuestion} />
          )}
        </div>
